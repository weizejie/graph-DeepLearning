...
for day in range(num_days):
    ...
    td = TemporalData(
        ...
        dst=torch.tensor(dst_list, dtype=torch.int64),
        t=torch.tensor(t_list, dtype=torch.int64),
        msg=torch.tensor(np.array(x_list),dtype=torch.float32)  # shape: [#edge, 3]
    )
    temporal_graphs.append(td)

with open('temporal_data_pems04.pkl', 'wb') as f:
...
torch.save(torch.tensor(labels_binary, dtype=torch.float32), 'pems04_binary_labels.pt')
...
torch.save(torch.tensor(labels_multi, dtype=torch.float32), 'pems04_multi_labels.pt')
