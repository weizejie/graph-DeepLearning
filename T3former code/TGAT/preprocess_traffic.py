...
for day in range(num_days):
    ...
    td = TemporalData(
        ...
        x=torch.tensor(np.array(x_feat),dtype=torch.float32)  # shape: [#edge, 3]
    )
...
def temporal_graph_to_data_list(temporal_graph,y):
    ...
    """
    x_all = temporal_graph.x  # shape: [N, T, F]
...
all_temporal_graphs_as_lists = [temporal_graph_to_data_list(g, y) for g, y in zip(temporal_graphs, labels_binary)]
with open('temporal_binary_pems08.pkl', 'wb') as f:
