...
def prep(data_dir, data_name):
    ...
    for idx, temporal_graph in enumerate(temporal_graphs):
        ...
        for i in range(args.num_sub_graphs):
            ...
            src = snapshot.src.tolist()
            dst = snapshot.dst.tolist()
            t = snapshot.t.tolist()

            ext_roll = [0] * (len(t) - 2) + [1] + [2]

            df_dict = {"src": src, "dst": dst, "time": t, "ext_roll": ext_roll}

            df = pd.DataFrame(df_dict)

            os.makedirs('./DATA/{}_{}/{}'.format(data_name, i, idx), exist_ok=True)
            file_path = "./DATA/{}_{}/{}".format(data_name, i, idx)

            df.to_csv(file_path + '/edges.csv', index = True)

if __name__ == "__main__":
    prep(data_dirs[0], 'pems08')
    #prep(data_dirs[0], data_names[0])
    # prep(data_dirs[1], data_names[1])
    # prep(data_dirs[2], data_names[2])
    # prep(data_dirs[3], data_names[3])
    # prep(data_dirs[4], data_names[4])
    # prep(data_dirs[-1], data_names[-1])
